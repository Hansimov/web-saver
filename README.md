...
- **内容哈希去重** — 基于图片内容哈希（cyrb53）检测重复，即使 URL 不同也能识别相同图片
- **持久化下载历史** — 跨会话记录已下载图片的哈希，避免重复下载
- **Shadow DOM 隔离** — 浮动按钮和 Toast 通过 Shadow DOM 渲染，不受页面样式干扰
...
| **重复图片**     | `skip`                          | `skip` = 跳过内容相同的图片；`latest` = 每次重新下载 |
...
通过图片内容哈希（cyrb53 算法）检测重复图片，即使不同 URL 指向相同内容也能识别：
...
- 下载历史通过 `GM_setValue` 持久化保存，跨会话有效。
- 使用 `GM_xmlhttpRequest` 获取图片数据（可跨域），计算内容哈希后再决定是否下载。
- 在设置面板中可点击「清除下载历史」按钮强制重新下载所有图片。
- 哈希存储上限为 5000 条，超过时自动清理最旧的记录。

> **注意**：由于浏览器沙箱限制，脚本无法检查本地文件是否存在。下载历史仅记录哈希与文件名的映射，若手动删除了已下载的文件，可通过「清除下载历史」强制重新下载。
...
| `GM_addStyle`                 | 注入图片高亮的 CSS 样式          |
| `GM_xmlhttpRequest`           | 跨域获取图片数据用于计算内容哈希 |
| `unsafeWindow`                | 在 Tampermonkey 沙箱中注册快捷键 |
